  ...
  <div class="page-wrapper">
    <v-layout row style="margin-top:100px">
      <v-flex xs12 md8 offset-md2 class="mt-3">
        ...
        <div class="date-area" v-if="viewarea == 'review'">
          <v-card>
            <v-card-title primary-title>
              <v-layout justify-center row>
                <div class="headline">Review Services</div>
              </v-layout>
            </v-card-title>
            <v-divider></v-divider>
            <v-card-body>
...
brief:"Camera Power Port Check, Adaptor Check, Connector Check"
categgory_name:"CCTV Services"
id:55
name:"Camera Power Problem"
price:"625.00"
qty:1
service_id:"27"
service_name:"Camera Problem"
                  unit_type:"Unit"-->
                  ...
                  <tbody>
                    <tr v-for="bit in selectedServiceBit" :key="bit.id">
                      <td>{{bit.service_name}}</td>
                      <td>{{bit.name}}</td>
                      <td>{{bit.price}}</td>
                      <td>{{bit.additional_price}}</td>
                      <td>{{bit.qty}}</td>
                      <td>{{bit.qty > 1 ? bit.price + (bit.additional_price*(bit.qty-1)) : bit.price}}</td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td>
                        <input
                          ...
                          class="form-control form-control-sm"
                          style="width:50%"
                          placeholder="Service Bit Price"
                          value="0"
                          v-model="customService.price"
                        />
                      </td>
                      <td>
                        <input
                          ...
                          class="form-control form-control-sm"
                          style="width:50%"
                          placeholder="Additional Service Bit Price"
                          value="0"
                          v-model="customService.additional_price"
                        />
                      </td>
                      <td>
                        <input
                      ...
                      <td>
                        <input
                          ...
                          :value="customService.qty > 1 ? customService.price + (customService.additional_price*(customService.qty - 1)) : customService.price"
                        />
                      </td>
                    </tr>
                    <v-btn small style="float:right" @click="customServiceAdd">নতুন যোগ করুন</v-btn>
                  </tbody>
                </table>
              </v-layout>
            </v-card-body>
...
export default {
  ...
  methods: {
    ...
    customServiceAdd() {
      if (this.customService.name) {
        this.selectedServiceBit.push(this.customService);
      }
      // this.customService.additional_price = "";
      // this.customService.brief = "";
      // this.customService.categgory_name = "";
      // this.customService.id = 0;
      // this.customService.name = "";
      // this.customService.price = 0;
      // this.customService.qty = 1;
      // this.customService.service_id = 0;
      // this.customService.service_name = "";
      // this.customService.unit_type = "";
    },
    ...
    nextView: function() {
      if (this.viewarea == "service") {
        this.viewarea = "review";
      } else if (this.viewarea == "review") {
