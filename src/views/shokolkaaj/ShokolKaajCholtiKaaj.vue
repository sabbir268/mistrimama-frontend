      ...
      >
        <template v-slot:items="props">
          ...
          <td
            ...
            style="cursor: pointer"
            @click="openDrawer(props.item)"
          >{{ props.item.address }}</td>
          <td
            class="text-xs-left"
            style="cursor: pointer"
            @click="openDrawer(props.item)"
          >{{props.item.date}}/{{props.item.time}}</td>
          <td
            class="text-xs-left"
            style="cursor: pointer"
            @click="openDrawer(props.item)"
          ...
          <td
            ...
            style="cursor: pointer"
            @click="openDrawer(props.item)"
          >{{props.item.comrade_phone}}</td>

          <td
            class="text-xs-left"
            style="cursor: pointer"
            @click="openDrawer(props.item)"
          >{{props.item.state = 1 ? 'সহকারি এলোকেটেড' : props.item.state = 2 ? 'কাজ চলছে' : props.item.state = 3 ? ' পেমেন্টের জন্য অপেক্ষামান' : '' }}</td>
...
export default {
  ...
  methods: {
    ...
    async getOrders() {
      var res = await axios.get("ongoing-order");
